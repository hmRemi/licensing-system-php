...
if (isset($_POST['newPlugin']) && isset($_POST['newLicense']) && isset($_POST['newDiscord']) && isset($_POST['newDiscriminator']) && isset($_POST['newIps']) && isset($_GET['pl']) && isset($_GET['license'])) {
    $newPl = $_POST['newPlugin'];
    $newKey = $_POST['newLicense'];
    $newDiscord = $_POST['newDiscord'] . "#" . $_POST['newDiscriminator'];
    $newIps = $_POST['newIps'];
    ...
    $oldPl = $_GET['pl'];
    $oldKey = $_GET['license'];
    $oldDc = getLicenseInfo($oldPl, $oldKey)->fetch_assoc()['DISCORD'];

    $updateStmt = $conn->prepare("UPDATE licenses SET DISCORD = ?, LICENSE = ?, PLUGIN = ?, `MAX-IPS` = ? WHERE PLUGIN = ?");
    $updateStmt->bind_param('sssis', $newDiscord, $newKey, $newPl, $newIps, $oldPl);
    $updateStmt->execute();

    $error = "<div class=\"alert alert-success\"><center>Successfully edited the license.<center></div>";
}
...
if (isset($_COOKIE['USER_TOKEN'])) {
    ...
    if (isset($_GET['pl']) && isset($_GET['license'])) {
        include '../utils/licenses.php';
        ...
        $licenseInfo = getLicenseInfo($plugin, $license)->fetch_assoc();
